...
export default function PhorestSettings() {
  ...
  const [selectedPhorestId, setSelectedPhorestId] = useState<string>('');
  const [phorestStaffName, setPhorestStaffName] = useState<string>('');
  ...
  const handleCreateMapping = async () => {
    if (!selectedUserId || !selectedPhorestId) {
      toast({
        title: 'Missing fields',
        description: 'Please select both a team member and enter a Phorest Staff ID.',
        variant: 'destructive',
      });
      return;
    }
    ...
    await createMapping.mutateAsync({
      ...
      phorest_staff_id: selectedPhorestId,
      phorest_staff_name: phorestStaffName || undefined,
    });

    setSelectedUserId('');
    setSelectedPhorestId('');
    setPhorestStaffName('');
  };
          ...
          <TabsContent value="mapping">
            <Card className="p-6">
              ...
              <p className="text-muted-foreground text-sm mb-6">
                Map your dashboard team members to their Phorest staff IDs to sync their performance data.
              </p>
              ...
              <div className="flex flex-col md:flex-row gap-4 mb-8 p-4 bg-muted rounded-lg">
                ...
                <input
                  type="text"
                  placeholder="Phorest Staff ID"
                  value={selectedPhorestId}
                  onChange={(e) => setSelectedPhorestId(e.target.value)}
                ...
                <input
                  ...
                  value={phorestStaffName}
                  onChange={(e) => setPhorestStaffName(e.target.value)}
                  className="flex h-10 w-full md:w-48 rounded-md border border-input bg-background px-3 py-2 text-sm"
                />

                <Button 
                  onClick={handleCreateMapping}
                  disabled={createMapping.isPending || !selectedUserId || !selectedPhorestId}
                >
