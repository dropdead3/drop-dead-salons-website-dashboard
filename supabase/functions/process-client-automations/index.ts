...
serve(async (req) => {
  ...
  try {
    ...
    for (const rule of rules as AutomationRule[]) {
      try {
        const ruleResults = await processRule(supabase, resend, rule, dryRun);
...
async function processRule(
  ...
  switch (rule.rule_type) {
    case 'post_visit_thanks':
      // Find clients who had appointments yesterday
      clientsToContact = await findRecentVisitors(supabase, rule.organization_id, targetDateStr);
      break;

    case 'rebooking_reminder':
      // Find clients whose last visit was X days ago and have no upcoming appointments
      clientsToContact = await findClientsNeedingRebooking(supabase, rule.organization_id, targetDateStr);
      break;

    case 'win_back':
      // Find inactive clients (no visit in X days)
      clientsToContact = await findInactiveClients(supabase, rule.organization_id, targetDateStr);
      break;

    case 'birthday':
      // Find clients with birthdays today or in coming days
      clientsToContact = await findBirthdayClients(supabase, rule.organization_id);
      break;
  ...
  for (const client of clientsToContact) {
    try {
      // Check if we already sent this automation to this client recently
      ...
      if (rule.email_template_id && resend) {
      ...
      } else if (!rule.email_template_id) {
        // No template configured, just log that we would have sent
        results.skipped += 1;
      }
...
async function findRecentVisitors(supabase: any, orgId: string, targetDate: string) {
  ...
  const { data: clients } = await supabase
    ...
    .select("id, phorest_client_id, first_name, last_name, email")
...
async function findClientsNeedingRebooking(supabase: any, orgId: string, targetDate: string) {
  // Find clients whose last appointment was around the target date
  const { data: lastAppointments } = await supabase
    .from("phorest_appointments")
    .select("phorest_client_id, appointment_date")
    .eq("appointment_date", targetDate)
    .not("status", "in", '("cancelled","no_show")');

  if (!lastAppointments || lastAppointments.length === 0) return [];

  const clientIds = [...new Set(lastAppointments.map((a: any) => a.phorest_client_id).filter(Boolean))];

  // Check which ones have future appointments
  ...
  const clientsWithFutureAppts = new Set(
    (futureAppointments || []).map((a: any) => a.phorest_client_id)
  );

  // Get clients without future appointments
  const clientsNeedingRebooking = clientIds.filter(id => !clientsWithFutureAppts.has(id));

  if (clientsNeedingRebooking.length === 0) return [];

  const { data: clients } = await supabase
    .from("phorest_clients")
    .select("id, phorest_client_id, first_name, last_name, email")
    .in("phorest_client_id", clientsNeedingRebooking);

  return clients || [];
}

async function findInactiveClients(supabase: any, orgId: string, targetDate: string) {
  // Find clients who haven't visited since target date
  ...
  const recentClientIds = new Set(
    (recentClients || []).map((a: any) => a.phorest_client_id).filter(Boolean)
  );

  // Get clients who have visited before but not recently
  const { data: allClientsWithVisits } = await supabase
    .from("phorest_appointments")
    .select("phorest_client_id")
    .lt("appointment_date", targetDate)
    .not("status", "in", '("cancelled","no_show")');

  const inactiveClientIds = [...new Set(
    (allClientsWithVisits || [])
      .map((a: any) => a.phorest_client_id)
      .filter((id: string) => id && !recentClientIds.has(id))
  )];
  ...
  const { data: clients } = await supabase
    .from("phorest_clients")
    .select("id, phorest_client_id, first_name, last_name, email")
    .in("phorest_client_id", inactiveClientIds.slice(0, 100)); // Limit to 100

  return clients || [];
}

async function findBirthdayClients(supabase: any, orgId: string) {
  const today = new Date();
  const month = today.getMonth() + 1;
  const day = today.getDate();

  // This would need date_of_birth field in phorest_clients
  // For now, return empty - would need schema update
  return [];
}
...
async function sendAutomationEmail(
  ...
  try {
    ...
    let subject = template.subject || "Message from Drop Dead Salons";
    ...
    await resend.emails.send({
      from: "Drop Dead Salons <noreply@notifications.dropdeadsalons.com>",
      to: client.email,
      subject,
      html: body,
    });
